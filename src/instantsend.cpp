...
bool CInstantSend::ProcessNewTxLockVote(CNode* pfrom, const CTxLockVote& vote, CConnman& connman)
{
    ...
    if(it == mapTxLockCandidates.end() || !it->second.txLockRequest) {
        ...
        auto itMnOV = mapDynodeOrphanVotes.find(vote.GetDynodeOutpoint());
        if(itMnOV == mapDynodeOrphanVotes.end()) {
            mapDynodeOrphanVotes.emplace(vote.GetDynodeOutpoint(), nDynodeOrphanExpireTime);
        } else {
            if(itMnOV->second > GetTime() && itMnOV->second > GetAverageDynodeOrphanVoteTime()) {
            ...
            itMnOV->second = nDynodeOrphanExpireTime;
        }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    dynode_info_t infoMn;

    if(!dnodeman.GetDynodeInfo(outpointDynode, infoMn)) {
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        if (!CHashSigner::VerifyHash(hash, infoMn.pubKeyDynode, vchDynodeSignature, strError)) {
            // could be a signature in old format
            std::string strMessage = txHash.ToString() + outpoint.ToStringShort();
            if(!CMessageSigner::VerifyMessage(infoMn.pubKeyDynode, vchDynodeSignature, strMessage, strError)) {
